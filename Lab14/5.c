...
node_t *Bind(node_t *k_head, node_t *n_head, int num, int n){
    int cnt = 1;
    node_t *cur = k_head;
    node_t *cur2 = n_head;
    while (cur->next != NULL && cnt != num && num <= n){
        cnt++;
        cur = cur->next;
    }
    cnt = 1;
    while(cur2->next != NULL && cnt != num && num <= n){
        cnt++;
        cur2 = cur2->next;
    }
    cur->prev = cur2;
...
int main(){
    ...
    int n;
    int k;
    printf("Введите число узлов в нижнем части списка: ");
    scanf("%d", &n);
    printf("Введите число узлов в верхней части списка: ");
    scanf("%d", &k);
    ...
    Bind(head_k, head, 1, n);
    for (int i = 1; i < k; i++){
        AddNode(head_k, NewNode(NewData(i)));
        Bind(head_k, head, i+1, n);
    }
